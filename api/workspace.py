...
회사 업무 영역 전용 API. 개인 영역(개인일정관리·가족메모·시간표·알림장/앨범)은
api/personal_workspace.py로 분리되어 있습니다 (Vercel Hobby 플랜의 서버리스
함수 개수 제한 12개를 맞추기 위해, hr_backup.py+hr_storage_backup.py를
하나로 합쳐서 확보한 자리를 이 분리에 씀).

화면(schedule.js/dashboard.js/hr.js)의 호출 주소는 예전 그대로
(/api/schedule, /api/contacts, /api/contract_docs, /api/daily_todos 등) 두고,
vercel.json의 routes 설정에서 각 주소를 "?resource=xxx" 파라미터를 붙여
이 파일 하나로 몰아줍니다.

- resource=schedule      -> 세무/업무 일정관리
- resource=contacts      -> 거래처 연락처
- resource=contractdocs  -> 계약/증빙 서류 관리
- resource=todos         -> 일자별 할일 메모
- resource=promotions    -> 인사기록보고서/승진이력/급여기준표
- resource=annualleave   -> 연차수당 자동계산
- resource=manuals       -> 급여/퇴직급여 업무 매뉴얼

모든 요청에 X-HR-Password 헤더 필요 (관리자 전용 — 가족 계정은 접근 불가).
"""
...
FAMILY_PASSWORD = os.environ.get("FAMILY_PASSWORD", "")  # auth_role()이 참조 — 가족 비밀번호가 들어와도 "family"로 정확히 판별해서 차단하기 위함
...
class handler(BaseHTTPRequestHandler):
    def _authorized(self, qs=None):
        """workspace.py는 회사 업무 전용이라 관리자(admin) 비밀번호만 허용.
        가족 비밀번호로 접근 가능한 개인 영역은 api/personal_workspace.py로 분리됨."""
        return auth_role(self.headers.get("X-HR-Password", "")) == "admin"
