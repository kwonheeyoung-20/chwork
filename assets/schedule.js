...
async function loadCalendar() {
  ...
  try {
    const [res] = await Promise.all([
      fetch(`${apiBase()}/api/schedule?from=${fromStr}&to=${toStr}&status=all&skip_prepare=1`, {
        headers: authHeaders(),
      }),
      ensureHolidaysLoaded(calYear),
    ]);
    if (handle401(res)) return;
    const data = await res.json();
...
// (개인일정관리의 personal.js에도 동일한 구조가 있음 — 서버 쪽 holidays 테이블을 공유함)
const holidayCache = {};
